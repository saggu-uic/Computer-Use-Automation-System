...
async def run_demo(evidence_dir: Path | None = None, headed: bool = False) -> int:
    ...
    try:
        ...
        if balance is None or transactions is None:
            failures.append("scripted discovery did not produce both capabilities")
            return 1
...
def cost_table(evidence: Path) -> str:
    ...
    lines += ["", "Scripted discovery rows have zero LLM cost by construction; live rows (`01a`-`01d`) show real model usage, including runs that produced no capability."]
    return "\n".join(lines) + "\n"
